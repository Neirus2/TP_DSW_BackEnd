...
function traverseForScope(path, visitors, state) {
  ...
  _traverse(path.parentPath, path.parent, path.node, path.container, path.key, path.listKey, path.hub);
  function _traverse(parentPath, parent, node, container, key, listKey, hub) {
    if (!node) {
      return;
    }
    const path = _index.NodePath.get({
      hub,
      parentPath,
      parent,
      container,
    ...
    (0, _context._forceSetScope)(path);
